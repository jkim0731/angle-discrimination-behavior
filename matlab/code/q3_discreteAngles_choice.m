...
clear
mdlName = {'mdlDiscreteExpertChoiceFT','mdlDiscreteExpertChoiceAllT'};


for g = 1:length(mdlName)
    load(['C:\Users\shires\Documents\GitHub\AngleDiscrimBehavior\matlab\datastructs\' mdlName{g}])
    figure(123+g);clf
    for i = 1:length(groupMdl)
        ...
        rawToPredRMSE(g,i) = sqrt(sum((predPsycho(:,1) - rawPsycho(:,1)).^2) ./ length(uVals)); %RMSE of average model and true
